...
public class ListEx {
	...
	/*
		...
		listEx(linkedList);
	*/
		// List(인터페이스)에는 실제 구현과는 별도로 List에서
		// 처리해야할 기능의 목록이 선언만 되어 있다.
		// 각각의 실체 클래스에서는 인터페이스를 구현한 실제 로직이 담겨 있다,
		List<String> arrayList = new ArrayList<>();
		listEx(arrayList);
	}
//	private static void listEx(LinkedList list) {
	private static void listEx(List list) {
		// List 는 인터페이스 -> 실체 클래스는 교체가 가능해진다.
		// 객체의 추가 : add
		list.add("Java"); // 리스트의 맨 마지막에 새 객체를 추가한다
		list.add("C");
		list.add("C++");
		list.add("Python");
