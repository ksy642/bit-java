...
//		stringMethods();
	...
	private static void stringMethods() {
		...
		// 검색 관련 : 문자, 문자열의 위치 찾을 때
		System.out.println("5번 인덱스의 문자 : " + s.charAt(5));
		// 중요 indexOf
		System.out.println("Fun 문자열의 위치 인덱스 : " 
				+ s.indexOf("Fun"));	// 검색어 대소문자를 구분!!
		System.out.println("fun 문자열의 위치 인덱스 : "
				+ s.indexOf("fun")); // -1 이라는 뜻은 찾을 수 없다는 뜻 !!!!!!!!!!!!
		System.out.println("m의 위치 인덱스 : " 
				+ s.indexOf("m"));  // → 가는거
		System.out.println("m의 역방향 위치 인덱스 : " 
				+ s.lastIndexOf("m")); // ← 이쪽으로 오는거
		System.out.println("m의 위치 인덱스 2nd : " 
				+ s.lastIndexOf("m", 12)); // 검색 시작 위치12번 인덱스부터 다시 검색해달라
		...
		// 변환 메소드를 수행해도 내부 데이터는 변경되지 않는다. (중요 : immutable)
		...
		// ABC   ABD
		// AB 는 둘다 같기 때문에 C와 D 를 비교한다.   D가 더크다 <
		// 같으면 0, 비교값이 작으면 음수, 크면 양수
		System.out.println("ABC".compareTo("ABD"));
		// ABC 가 작기에 -1 로 나와야 정상		
	}
